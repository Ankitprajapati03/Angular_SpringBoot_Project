...
export class RegistrationComponent {
  ...
  errorMessageAdditionalDocument: string = '';
  ...
  validateStudentPhoto() {
    if (this.studentPhoto) {
      ...
      if (!validImageTypes.includes(this.studentPhoto.type)) {
        this.errorMessageFile = 'Photo must be JPEG, PNG, or GIF';
  ...
  validateAdditionalDocument() {
    if (this.additionalDocument) {
      ...
      if (!validDocTypes.includes(this.additionalDocument.type)) {
        this.errorMessageAdditionalDocument = 'Document must be PDF, Word, Excel, or image';
        this.additionalDocument = null;
      } else {
        this.errorMessageAdditionalDocument = '';
      }
    }
  }
  ...
  validateForm(): boolean {
    ...
    if (this.errorMessage || this.errorMessageName || this.errorMessageRollNumber || this.errorMessageFile || this.errorMessageAdditionalDocument) {
      return false;
    }
